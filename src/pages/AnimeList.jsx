...
const AnimeList = () => {
  ...
  return (
    <main className="pt-26">
      <div className="lg:m-auto px-5 mx-5 py-5 max-w-6xl bg-neutral-900 border border-1 border-neutral-700/60 rounded-xl">
        ...
        {loading ? (
          ...
          <>
            <section className="mt-7 flex gap-3 flex-wrap">
              {animes.map((data, i) => (
                <Link className="border border-neutral-600 hover:bg-yellow-500 cursor-pointer duration-200 py-1 px-3" key={i} to={data.startWith} smooth={true} spy={true} offset={-200} duration={1000}>
                  {data.startWith}
                </Link>
              ))}
            </section>
                ...
                <>
                  ...
                  <ul id={data.startWith} key={i} className="border-b-1 flex  flex-wrap md:flex-row flex-col w-full border-neutral-700 pb-2">
                    {data.animeList.map((anime, i) => (
                      <li className="py-1 w-[48%] text-sm hover:text-yellow-300 cursor-pointer line-clamp-1 text-elipsis ">{anime.title}</li>
                    ))}
                  </ul>
                </>
              ))}
